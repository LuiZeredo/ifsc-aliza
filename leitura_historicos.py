...
# Otimizadas para layout IFSC. O '^\\s*' lida com espaços em branco no início da linha.
REGEX_DADOS = {
...
# Regex para a tabela de disciplinas. Captura (Semestre, Num1, Num2, Situação).
# DOTALL permite que a busca ignore quebras de linha dentro das células da tabela.
...
def calcular_idade(data_nasc_str):
    ...
    try:
        ...
        hoje = datetime.now()
        # Cálculo preciso (Ano atual - Ano nasc - (Se o aniversário ainda não passou))
...
def calcular_metricas_recentes(disciplinas):
    ...
    for i, nome_sem in enumerate(['Ultimo', 'Penultimo', 'Antepenultimo']):
        if len(semestres_unicos) >= (i + 1):
            sem_key = semestres_unicos[-(i + 1)]
            metricas = _calcular_metricas_semestre(disciplinas[sem_key])
        else:
...
def processar_historico_pdf(caminho_pdf):
    ...
    # NOVO: Calcula Idade (Substitui 'Data_Nascimento')
    ...
    for match in matches:
        try:
            ...
            if num1 <= 10.0 and num2 > 10.0:
                ...
                nota, freq = num2, num1
            elif num1 >= 70.0 and num2 <= 10.0: # Fallback p/ (7.0 e 80.0) ou (10.0 e 100.0)
                freq, nota = num1, num2
    ...
    # 3. Calcula métricas recentes (Engenharia de Features)
